...
public class ConstraintWidgetContainer extends WidgetContainer {
    ...
    @Override // androidx.constraintlayout.core.widgets.ConstraintWidget, androidx.constraintlayout.core.widgets.WidgetContainer
    public void reset() {
        this.mSystem.reset();
        this.mPaddingLeft = 0;
    ...
    public boolean addChildrenToSolver(LinearSystem system) {
        ...
        if (LinearSystem.USE_DEPENDENCY_ORDERING) {
            ...
            addChildrenToSolverByDependency(this, system, widgetsToAdd2, getHorizontalDimensionBehaviour() == ConstraintWidget.DimensionBehaviour.WRAP_CONTENT ? 0 : 1, false);
    ...
    public void layout() {
        ...
        if (this.pass == 0 && Optimizer.enabled(this.mOptimizationLevel, 1)) {
            ...
            int i2 = 0;
            while (i2 < count) {
                ConstraintWidget child = (ConstraintWidget) this.mChildren.get(i2);
                if (child.isMeasureRequested() && !(child instanceof Guideline) && !(child instanceof Barrier) && !(child instanceof VirtualLayout) && !child.isInVirtualLayout()) {
                    if (!(child.getDimensionBehaviour(0) == ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT && child.mMatchConstraintDefaultWidth != i && child.getDimensionBehaviour(i) == ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT && child.mMatchConstraintDefaultHeight != i)) {
                        measure(0, child, this.mMeasurer, new BasicMeasure.Measure(), BasicMeasure.Measure.SELF_DIMENSIONS);
                    }
                }
                i2++;
                i = 1;
        ...
        resetChains();
        for (int i3 = 0; i3 < count; i3++) {
            ConstraintWidget widget = (ConstraintWidget) this.mChildren.get(i3);
        ...
        while (needsSolving4) {
            ...
            if (needsSolving4) {
                ...
                for (int i5 = 0; i5 < count; i5++) {
                    ((ConstraintWidget) this.mChildren.get(i5)).updateFromSolver(this.mSystem, optimize);
                }
                needsSolving = false;
            }
            if (!hasWrapContent || countSolve2 >= 8 || !Optimizer.flags[2]) {
                ...
                int i6 = 0;
                while (i6 < count) {
                    ConstraintWidget widget2 = (ConstraintWidget) this.mChildren.get(i6);
                    maxX = Math.max(maxX, widget2.f38mX + widget2.getWidth());
                    maxY = Math.max(maxY, widget2.f39mY + widget2.getHeight());
                    i6++;
                ...
                if (originalVerticalDimensionBehaviour == ConstraintWidget.DimensionBehaviour.WRAP_CONTENT && getHeight() < maxY2) {
                    ...
                    if (wrap_override == 0) {
                        if (this.mListDimensionBehaviors[0] == ConstraintWidget.DimensionBehaviour.WRAP_CONTENT && preH2 > 0 && getWidth() > preH2) {
                            this.mWidthMeasuredTooSmall = true;
                            wrap_override = 1;
                            this.mListDimensionBehaviors[0] = ConstraintWidget.DimensionBehaviour.FIXED;
                            setWidth(preH2);

...
public class Direct {
            ...
            if (left.getDependents() != null && left.hasFinalValue()) {
                ...
                while (it.hasNext()) {
                    ...
                    if (widget.isMeasureRequested() && canMeasure) {
                        ConstraintWidgetContainer.measure(level + 1, widget, measurer, new BasicMeasure.Measure(), BasicMeasure.Measure.SELF_DIMENSIONS);
                    }
                ...
                if (right.getDependents() != null && right.hasFinalValue()) {
                    ...
                    while (it2.hasNext()) {
                        ...
                        if (widget2.isMeasureRequested() && canMeasure2) {
                            ConstraintWidgetContainer.measure(level + 1, widget2, measurer, new BasicMeasure.Measure(), BasicMeasure.Measure.SELF_DIMENSIONS);
                        }
                ...
                if (baseline.getDependents() != null && baseline.hasFinalValue()) {
                    ...
                    while (it3.hasNext()) {
                        ...
                        if (widget3.isMeasureRequested() && canMeasure3) {
                            ConstraintWidgetContainer.measure(level + 1, widget3, measurer, new BasicMeasure.Measure(), BasicMeasure.Measure.SELF_DIMENSIONS);
                        }
    ...
    /* JADX INFO: Multiple debug info for r10v7 'measure'  androidx.constraintlayout.core.widgets.analyzer.BasicMeasure$Measure: [D('begin' androidx.constraintlayout.core.widgets.ConstraintAnchor), D('measure' androidx.constraintlayout.core.widgets.analyzer.BasicMeasure$Measure)] */
                    ...
                    if (end.mTarget.hasFinalValue()) {
                        if (firstVisibleWidget != null) {
                            if (lastVisibleWidget != null) {
                                ...
                                while (!done3) {
                                    if (!canMeasure(0 + 1, widget2) || widget2.mListDimensionBehaviors[orientation] == ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT) {
                                        return false;
                                    }
                                    if (widget2.isMeasureRequested()) {
                                        done2 = done3;

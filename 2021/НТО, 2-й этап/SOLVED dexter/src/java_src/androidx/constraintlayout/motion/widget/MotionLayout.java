...
public class MotionLayout extends ConstraintLayout implements NestedScrollingParent3 {
    ...
    public void setState(TransitionState newState) {
        if (newState != TransitionState.FINISHED || this.mCurrentState != -1) {
            ...
            switch (C01565.f51xabc7e4ac[oldState.ordinal()]) {
                case 1:
                case 2:
    ...
    public static /* synthetic */ class C01565 {
            ...
            try {
                ...
                try {
                    ...
                    if (stateCache == null) {
                        ...
                        post(new Runnable() {
                            /* class androidx.constraintlayout.motion.widget.MotionLayout.RunnableC01521 */
    ...
    public void rotateTo(int id, int duration) {
        ...
        transitionToEnd(new Runnable() {
            /* class androidx.constraintlayout.motion.widget.MotionLayout.RunnableC01532 */
    ...
    public void transitionToState(int id, int screenWidth, int screenHeight, int duration) {
        ...
        if (motionScene == null || motionScene.mStateSet == null) {
            ...
            i4 = id;
            int tmp_id = this.mScene.mStateSet.convertToConstraintSet(this.mCurrentState, i4, (float) screenWidth, (float) screenHeight);
            if (tmp_id != -1) {
                id2 = tmp_id;
    ...
    public class Model {
        ...
        public void build() {
            ...
            int i6 = 0;
            while (i6 < n) {
                MotionController controller = controllers3.get(ids[i6]);
                int relativeToId = controller.getAnimateRelativeTo();
                if (relativeToId != -1) {
                    controller.setupRelative(controllers3.get(relativeToId));
                }
                i6++;
                controllers3 = controllers3;
            }
        }
    ...
    /* JADX INFO: Multiple debug info for r6v4 float: [D('dir' float), D('progress' float)] */
    @Override // androidx.core.view.NestedScrollingParent2
        ...
        if (scene != null && (currentTransition = scene.mCurrentTransition) != null && currentTransition.isEnabled()) {
            if (!currentTransition.isEnabled() || (touchResponse = currentTransition.getTouchResponse()) == null || (regionId = touchResponse.getTouchRegionId()) == -1 || target.getId() == regionId) {
                ...
                if (!(currentTransition.getTouchResponse() == null || (currentTransition.getTouchResponse().getFlags() & 1) == 0)) {
                    float dir = scene.getProgressDirection((float) dx, (float) dy);
                    float f2 = this.mTransitionLastPosition;
                    if ((f2 <= 0.0f && dir < 0.0f) || (f2 >= 1.0f && dir > 0.0f)) {
                        if (Build.VERSION.SDK_INT >= 21) {
                            target.setNestedScrollingEnabled(false);
                            target.post(new Runnable() {
                                /* class androidx.constraintlayout.motion.widget.MotionLayout.RunnableC01543 */
                ...
                scene.processScrollMove((float) dx, (float) dy);
        ...
        if (this.mKeepAnimating || (this.mInTransition && (force || this.mTransitionGoalPosition != f))) {
            ...
            if (interpolator == null || done) {
                ...
                if (interpolator2 == stopLogic) {
                    stopLogicDone2 = stopLogic.isStopped() ? 2 : 1;
                } else {
                    stopLogicDone2 = 0;
                }
                ...
                if (interpolator4 instanceof MotionInterpolator) {
                    ...
                    this.mLastVelocity = ((interpolator4.getInterpolation(position + deltaPos) - position) * dir) / deltaPos;
                }
                stopLogicDone = 0;
            }
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs, C0172R.styleable.MotionLayout);
            int N = a.getIndexCount();
            boolean apply = true;
            for (int i = 0; i < N; i++) {
                int attr = a.getIndex(i);
                if (attr == C0172R.styleable.MotionLayout_layoutDescription) {
                    this.mScene = new MotionScene(getContext(), this, a.getResourceId(attr, -1));
                } else if (attr == C0172R.styleable.MotionLayout_currentState) {
                    this.mCurrentState = a.getResourceId(attr, -1);
                } else if (attr == C0172R.styleable.MotionLayout_motionProgress) {
                    this.mTransitionGoalPosition = a.getFloat(attr, 0.0f);
                    this.mInTransition = true;
                } else if (attr == C0172R.styleable.MotionLayout_applyMotionScene) {
                    apply = a.getBoolean(attr, apply);
                } else {
                    int i2 = 0;
                    if (attr == C0172R.styleable.MotionLayout_showPaths) {
                    ...
                    } else if (attr == C0172R.styleable.MotionLayout_motionDebug) {
    ...
    public void onAttachedToWindow() {
        ...
        if (stateCache == null) {
            ...
            post(new Runnable() {
                /* class androidx.constraintlayout.motion.widget.MotionLayout.RunnableC01554 */
    ...
    public void updateStateAnimate(int stateId, ConstraintSet set, int duration) {
        if (this.mScene != null && this.mCurrentState == stateId) {
            updateState(C0172R.C0175id.view_transition, getConstraintSet(stateId));
            setState(C0172R.C0175id.view_transition, -1, -1);
            updateState(stateId, set);
            MotionScene.Transition tmpTransition = new MotionScene.Transition(-1, this.mScene, C0172R.C0175id.view_transition, stateId);

...
public class TouchResponse {
    ...
    private void fillFromAttributeList(Context context, AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs, C0172R.styleable.OnSwipe);
        fill(a);
        a.recycle();
    }
    ...
    private void fill(TypedArray a) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (attr == C0172R.styleable.OnSwipe_touchAnchorId) {
                this.mTouchAnchorId = a.getResourceId(attr, this.mTouchAnchorId);
            } else if (attr == C0172R.styleable.OnSwipe_touchAnchorSide) {
            ...
            } else if (attr == C0172R.styleable.OnSwipe_dragDirection) {
            ...
            } else if (attr == C0172R.styleable.OnSwipe_maxVelocity) {
                this.mMaxVelocity = a.getFloat(attr, this.mMaxVelocity);
            } else if (attr == C0172R.styleable.OnSwipe_maxAcceleration) {
                this.mMaxAcceleration = a.getFloat(attr, this.mMaxAcceleration);
            } else if (attr == C0172R.styleable.OnSwipe_moveWhenScrollAtTop) {
                this.mMoveWhenScrollAtTop = a.getBoolean(attr, this.mMoveWhenScrollAtTop);
            } else if (attr == C0172R.styleable.OnSwipe_dragScale) {
                this.mDragScale = a.getFloat(attr, this.mDragScale);
            } else if (attr == C0172R.styleable.OnSwipe_dragThreshold) {
                this.mDragThreshold = a.getFloat(attr, this.mDragThreshold);
            } else if (attr == C0172R.styleable.OnSwipe_touchRegionId) {
                this.mTouchRegionId = a.getResourceId(attr, this.mTouchRegionId);
            } else if (attr == C0172R.styleable.OnSwipe_onTouchUp) {
                this.mOnTouchUp = a.getInt(attr, this.mOnTouchUp);
            } else if (attr == C0172R.styleable.OnSwipe_nestedScrollFlags) {
                this.mFlags = a.getInteger(attr, 0);
            } else if (attr == C0172R.styleable.OnSwipe_limitBoundsTo) {
                this.mLimitBoundsTo = a.getResourceId(attr, 0);
            } else if (attr == C0172R.styleable.OnSwipe_rotationCenterId) {
                this.mRotationCenterId = a.getResourceId(attr, this.mRotationCenterId);
            } else if (attr == C0172R.styleable.OnSwipe_springDamping) {
                this.mSpringDamping = a.getFloat(attr, this.mSpringDamping);
            } else if (attr == C0172R.styleable.OnSwipe_springMass) {
                this.mSpringMass = a.getFloat(attr, this.mSpringMass);
            } else if (attr == C0172R.styleable.OnSwipe_springStiffness) {
                this.mSpringStiffness = a.getFloat(attr, this.mSpringStiffness);
            } else if (attr == C0172R.styleable.OnSwipe_springStopThreshold) {
                this.mSpringStopThreshold = a.getFloat(attr, this.mSpringStopThreshold);
            } else if (attr == C0172R.styleable.OnSwipe_springBoundary) {
                this.mSpringBoundary = a.getInt(attr, this.mSpringBoundary);
            } else if (attr == C0172R.styleable.OnSwipe_autoCompleteMode) {
    ...
    public void processTouchRotateEvent(MotionEvent event, MotionLayout.MotionTracker velocityTracker, int currentState, MotionScene motionScene) {
        ...
        switch (event.getAction()) {
            ...
            case 2:
                ...
                float drag = (float) (((angle12 - Math.atan2((double) (this.mLastTouchY - rcy2), (double) (this.mLastTouchX - rcx2))) * 180.0d) / 3.141592653589793d);
                if (drag > 330.0f) {
                    drag -= 360.0f;
                } else if (drag < -330.0f) {
                    drag += 360.0f;
                ...
                if (((double) Math.abs(drag)) > 0.01d || this.mDragStarted) {
                    ...
                    float pos3 = Math.max(Math.min(pos2 + ((this.mDragScale * drag) / this.mAnchorDpDt[c]), 1.0f), 0.0f);
    ...
    public void setupTouch() {
        ...
        if (view instanceof NestedScrollView) {
            ...
            sv.setOnTouchListener(new View.OnTouchListener() {
                /* class androidx.constraintlayout.motion.widget.TouchResponse.View$OnTouchListenerC01581 */
            ...
            sv.setOnScrollChangeListener(new NestedScrollView.OnScrollChangeListener() {
                /* class androidx.constraintlayout.motion.widget.TouchResponse.C01592 */

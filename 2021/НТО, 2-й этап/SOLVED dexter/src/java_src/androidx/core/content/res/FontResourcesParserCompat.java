...
public class FontResourcesParserCompat {
    ...
    private static FamilyResourceEntry readFamily(XmlPullParser parser, Resources resources) throws XmlPullParserException, IOException {
        TypedArray array = resources.obtainAttributes(Xml.asAttributeSet(parser), C0183R.styleable.FontFamily);
        String authority = array.getString(C0183R.styleable.FontFamily_fontProviderAuthority);
        String providerPackage = array.getString(C0183R.styleable.FontFamily_fontProviderPackage);
        String query = array.getString(C0183R.styleable.FontFamily_fontProviderQuery);
        int certsId = array.getResourceId(C0183R.styleable.FontFamily_fontProviderCerts, 0);
        int strategy = array.getInteger(C0183R.styleable.FontFamily_fontProviderFetchStrategy, 1);
        int timeoutMs = array.getInteger(C0183R.styleable.FontFamily_fontProviderFetchTimeout, DEFAULT_TIMEOUT_MILLIS);
        String systemFontFamilyName = array.getString(C0183R.styleable.FontFamily_fontProviderSystemFontFamily);
        array.recycle();
    ...
    private static FontFileResourceEntry readFont(XmlPullParser parser, Resources resources) throws XmlPullParserException, IOException {
        ...
        int resourceAttr;
        TypedArray array = resources.obtainAttributes(Xml.asAttributeSet(parser), C0183R.styleable.FontFamilyFont);
        if (array.hasValue(C0183R.styleable.FontFamilyFont_fontWeight)) {
            weightAttr = C0183R.styleable.FontFamilyFont_fontWeight;
        } else {
            weightAttr = C0183R.styleable.FontFamilyFont_android_fontWeight;
        }
        int weight = array.getInt(weightAttr, NORMAL_WEIGHT);
        if (array.hasValue(C0183R.styleable.FontFamilyFont_fontStyle)) {
            styleAttr = C0183R.styleable.FontFamilyFont_fontStyle;
        } else {
            styleAttr = C0183R.styleable.FontFamilyFont_android_fontStyle;
        }
        boolean isItalic = 1 == array.getInt(styleAttr, 0);
        if (array.hasValue(C0183R.styleable.FontFamilyFont_ttcIndex)) {
            ttcIndexAttr = C0183R.styleable.FontFamilyFont_ttcIndex;
        } else {
            ttcIndexAttr = C0183R.styleable.FontFamilyFont_android_ttcIndex;
        }
        if (array.hasValue(C0183R.styleable.FontFamilyFont_fontVariationSettings)) {
            variationSettingsAttr = C0183R.styleable.FontFamilyFont_fontVariationSettings;
        } else {
            variationSettingsAttr = C0183R.styleable.FontFamilyFont_android_fontVariationSettings;
        }
        String variationSettings = array.getString(variationSettingsAttr);
        int ttcIndex = array.getInt(ttcIndexAttr, 0);
        if (array.hasValue(C0183R.styleable.FontFamilyFont_font)) {
            resourceAttr = C0183R.styleable.FontFamilyFont_font;
        } else {
            resourceAttr = C0183R.styleable.FontFamilyFont_android_font;
        }

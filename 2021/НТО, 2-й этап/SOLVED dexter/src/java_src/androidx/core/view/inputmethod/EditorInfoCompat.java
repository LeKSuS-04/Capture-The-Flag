...
public final class EditorInfoCompat {
    ...
    private static void trimLongSurroundingText(EditorInfo editorInfo, CharSequence subText, int selStart, int selEnd) {
        ...
        int newAfterCursorLength = Math.min(subText.length() - selEnd, maxLengthMinusSelection - Math.min(selStart, (int) (((double) maxLengthMinusSelection) * 0.8d)));
        ...
        if (newSelLength != sourceSelLength) {
            newInitialSurroundingText = TextUtils.concat(subText.subSequence(newBeforeCursorHead, newBeforeCursorHead + newBeforeCursorLength), subText.subSequence(selEnd, selEnd + newAfterCursorLength));
        } else {

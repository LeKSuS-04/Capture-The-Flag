...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() {
        /* class androidx.recyclerview.widget.DiffUtil.C04221 */
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (snake != null) {
                ...
                stack.add(left);
                if (!snake.reverse) {
                    range.oldListStart = snake.f77x + snake.size;
                    range.newListStart = snake.f78y + snake.size;
                } else if (snake.removal) {
                    range.oldListStart = snake.f77x + snake.size + 1;
                    range.newListStart = snake.f78y + snake.size;
                } else {
                    range.oldListStart = snake.f77x + snake.size;
                    range.newListStart = snake.f78y + snake.size + 1;
                }
                stack.add(range);
            } else {
                rangePool.add(range);
        ...
        for (int d = 0; d <= dLimit; d++) {
            for (int k = -d; k <= d; k += 2) {
                ...
                if (checkInFwd && k >= (delta - d) + 1 && k <= (delta + d) - 1 && forward[kOffset + k] >= backward[kOffset + k]) {
                    ...
                    outSnake.f77x = backward[kOffset + k];
            ...
            while (k2 <= d) {
                ...
                if (checkInFwd || k2 + delta < (-d) || k2 + delta > d || forward[kOffset + backwardK] < backward[kOffset + backwardK]) {
                    ...
                    outSnake2.f77x = backward[kOffset + backwardK];
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                for (int i = snakeSize - 1; i >= 0; i--) {
                    if ((this.mOldItemStatuses[snake.f77x + i] & 31) == 2) {
                        batchingCallback.onChanged(snake.f77x + i, 1, this.mCallback.getChangePayload(snake.f77x + i, snake.f78y + i));
                    }
                }
                posOld = snake.f77x;
                posNew = snake.f78y;

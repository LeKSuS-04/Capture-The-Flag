...
public abstract class VersionedParcel {
    ...
    private <T> void writeCollection(Collection<T> val) {
        ...
        if (n > 0) {
            ...
            switch (type) {
                ...
                case 7:
                    for (T v6 : val) {
                        writeInt(v6.intValue());
                    }
                    return;
                case 8:
                    for (T v7 : val) {
                        writeFloat(v7.floatValue());
                    }
                    return;
            }
        }
    ...
    /* JADX DEBUG: Multi-variable search result rejected for r1v0, resolved type: androidx.collection.ArrayMap */
    /* JADX WARN: Multi-variable type inference failed */
    ...
    public Serializable readSerializable() {
        ...
        try {
            return (Serializable) new ObjectInputStream(new ByteArrayInputStream(readByteArray())) {
                /* class androidx.versionedparcelable.VersionedParcel.C05181 */
    ...
    public <T extends VersionedParcelable> T readFromParcel(String parcelCls, VersionedParcel versionedParcel) {
        try {
            return (T) ((VersionedParcelable) getReadMethod(parcelCls).invoke(null, versionedParcel));

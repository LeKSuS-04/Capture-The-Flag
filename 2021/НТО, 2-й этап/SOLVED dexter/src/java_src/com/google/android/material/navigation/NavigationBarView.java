...
public abstract class NavigationBarView extends FrameLayout {
    ...
    public NavigationBarView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        TintTypedArray attributes = ThemeEnforcement.obtainTintedStyledAttributes(context2, attrs, C0552R.styleable.NavigationBarView, defStyleAttr, defStyleRes, C0552R.styleable.NavigationBarView_itemTextAppearanceInactive, C0552R.styleable.NavigationBarView_itemTextAppearanceActive);
        ...
        if (attributes.hasValue(C0552R.styleable.NavigationBarView_itemIconTint)) {
            createNavigationBarMenuView.setIconTintList(attributes.getColorStateList(C0552R.styleable.NavigationBarView_itemIconTint));
        } else {
            createNavigationBarMenuView.setIconTintList(createNavigationBarMenuView.createDefaultColorStateList(16842808));
        }
        setItemIconSize(attributes.getDimensionPixelSize(C0552R.styleable.NavigationBarView_itemIconSize, getResources().getDimensionPixelSize(C0552R.dimen.mtrl_navigation_bar_item_default_icon_size)));
        if (attributes.hasValue(C0552R.styleable.NavigationBarView_itemTextAppearanceInactive)) {
            setItemTextAppearanceInactive(attributes.getResourceId(C0552R.styleable.NavigationBarView_itemTextAppearanceInactive, 0));
        }
        if (attributes.hasValue(C0552R.styleable.NavigationBarView_itemTextAppearanceActive)) {
            setItemTextAppearanceActive(attributes.getResourceId(C0552R.styleable.NavigationBarView_itemTextAppearanceActive, 0));
        }
        if (attributes.hasValue(C0552R.styleable.NavigationBarView_itemTextColor)) {
            setItemTextColor(attributes.getColorStateList(C0552R.styleable.NavigationBarView_itemTextColor));
        }
        if (getBackground() == null || (getBackground() instanceof ColorDrawable)) {
            ViewCompat.setBackground(this, createMaterialShapeDrawableBackground(context2));
        }
        if (attributes.hasValue(C0552R.styleable.NavigationBarView_elevation)) {
            setElevation((float) attributes.getDimensionPixelSize(C0552R.styleable.NavigationBarView_elevation, 0));
        }
        DrawableCompat.setTintList(getBackground().mutate(), MaterialResources.getColorStateList(context2, attributes, C0552R.styleable.NavigationBarView_backgroundTint));
        setLabelVisibilityMode(attributes.getInteger(C0552R.styleable.NavigationBarView_labelVisibilityMode, -1));
        int itemBackground = attributes.getResourceId(C0552R.styleable.NavigationBarView_itemBackground, 0);
        if (itemBackground != 0) {
            createNavigationBarMenuView.setItemBackgroundRes(itemBackground);
        } else {
            setItemRippleColor(MaterialResources.getColorStateList(context2, attributes, C0552R.styleable.NavigationBarView_itemRippleColor));
        }
        if (attributes.hasValue(C0552R.styleable.NavigationBarView_menu)) {
            inflateMenu(attributes.getResourceId(C0552R.styleable.NavigationBarView_menu, 0));
        }
        attributes.recycle();
        addView(createNavigationBarMenuView);
        navigationBarMenu.setCallback(new MenuBuilder.Callback() {
            /* class com.google.android.material.navigation.NavigationBarView.C06731 */
    ...
    private void applyWindowInsets() {
        ViewUtils.doOnApplyWindowInsets(this, new ViewUtils.OnApplyWindowInsetsListener() {
            /* class com.google.android.material.navigation.NavigationBarView.C06742 */
    ...
    public static class SavedState extends AbsSavedState {
        public static final Parcelable.Creator<SavedState> CREATOR = new Parcelable.ClassLoaderCreator<SavedState>() {
            /* class com.google.android.material.navigation.NavigationBarView.SavedState.C06751 */

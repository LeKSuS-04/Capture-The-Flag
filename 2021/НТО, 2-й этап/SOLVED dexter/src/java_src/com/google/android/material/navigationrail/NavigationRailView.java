...
public class NavigationRailView extends NavigationBarView {
    ...
    public NavigationRailView(Context context, AttributeSet attrs) {
        this(context, attrs, C0552R.attr.navigationRailStyle);
    }

    public NavigationRailView(Context context, AttributeSet attrs, int defStyleAttr) {
        this(context, attrs, defStyleAttr, C0552R.style.Widget_MaterialComponents_NavigationRailView);
    }

    public NavigationRailView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        this.topMargin = getResources().getDimensionPixelSize(C0552R.dimen.mtrl_navigation_rail_margin);
        TintTypedArray attributes = ThemeEnforcement.obtainTintedStyledAttributes(getContext(), attrs, C0552R.styleable.NavigationRailView, defStyleAttr, defStyleRes, new int[0]);
        int headerLayoutRes = attributes.getResourceId(C0552R.styleable.NavigationRailView_headerLayout, 0);
        if (headerLayoutRes != 0) {
            addHeaderView(headerLayoutRes);
        }
        setMenuGravity(attributes.getInt(C0552R.styleable.NavigationRailView_menuGravity, 49));
        attributes.recycle();
    }

...
public final class CircularDrawingDelegate extends DrawingDelegate<CircularProgressIndicatorSpec> {
    ...
    private void drawRoundedEnd(Canvas canvas, Paint paint, float trackSize, float cornerRadius, float positionInDeg, boolean isStartPosition, RectF cornerPatternRectBound) {
        ...
        canvas.drawRect((this.adjustedRadius - (trackSize / 2.0f)) + cornerRadius, Math.min(0.0f, startOrEndFactor * cornerRadius * ((float) this.arcDirectionFactor)), (this.adjustedRadius + (trackSize / 2.0f)) - cornerRadius, Math.max(0.0f, startOrEndFactor * cornerRadius * ((float) this.arcDirectionFactor)), paint);
        canvas.translate((this.adjustedRadius - (trackSize / 2.0f)) + cornerRadius, 0.0f);
        canvas.drawArc(cornerPatternRectBound, 180.0f, (-startOrEndFactor) * 90.0f * ((float) this.arcDirectionFactor), true, paint);
        canvas.translate(trackSize - (cornerRadius * 2.0f), 0.0f);
        canvas.drawArc(cornerPatternRectBound, 0.0f, 90.0f * startOrEndFactor * ((float) this.arcDirectionFactor), true, paint);
        canvas.restore();
    }
}
